...
async function loginAndGetNonce() {
  ...
  const dashboardResponse = await fetch('https://the.invisimple.id/dashboard/invitation/create/', {
    headers: {
      'Cookie': cookies,
      'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
    },
  ...
  const nonceMatch = dashboardHtml.match(/__nonce["\s:=]+([a-f0-9]{10})/i) ||
                     dashboardHtml.match(/nonce["\s:=]+([a-f0-9]{10})/i);
  
  let extractedNonce = nonceMatch ? nonceMatch[1] : loginNonce;

  if (!extractedNonce) {
    console.log('Could not extract nonce from HTML, using login nonce as fallback');
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const now = Date.now();
    if (!cachedCookies || !cacheTimestamp || (now - cacheTimestamp > CACHE_DURATION)) {
    ...
    if (data.success === false && (data.data?.includes('Nonce') || data.data?.includes('nonce'))) {
